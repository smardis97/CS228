...
def new_char(engine, gui, mstate):
    gui.state_change(MENU_NEW_CHAR)


def text_confirm(engine, gui, mstate):
    if mstate == MENU_LOGIN or mstate == MENU_NEW_CHAR:
        name = ""
...
def settings(engine, gui, mstate):
    # gui.state_change(MENU_SETTINGS)
    print "SETTINGS"
